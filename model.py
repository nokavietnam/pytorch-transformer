...
class FeedForwardBlock(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, dropout: float) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(d_model, d_ff)  # W1 as B1
        self.dropout = nn.Dropout(dropout)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.dropout = nn.Dropout(dropout)
        # self.attention = FeedForwardBlock(d_model, d_model)
    ...
    def forward(self, q, k, v, mask=None):
        query = self.w_q(q)  # (batch, seq_len, d_model) --> (bathc, seq_len, d_model)
        key = self.w_k(k)  # (batch, seq_len, d_model) --> (bathc, seq_len, d_model)
        value = self.w_v(v)  # (batch, seq_len, d_model) --> (bathc, seq_len, d_model)
