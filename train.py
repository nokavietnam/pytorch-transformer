...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        # Build mask for the target (decoder layer)
...
def get_ds(config):
    ds_raw = load_dataset('harouzie/vi_en-translation', f'default', split='train')
    ...
    train_ds = BilingualDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'],
                                config['seq_len'])
    val_ds = BilingualDataset(valid_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'],
                              config['seq_len'])
    ...
    print(f'Max lenghth of source sentence: {max_len_src}')
    print(f'Max lenghth of target sentence: {max_len_tgt}')
...
def get_model(config, vocab_src_len, config_tgt_len):
    model = build_transformer(vocab_src_len, config_tgt_len, config['seq_len'], config['seq_len'], config['d_model'])
    return model
